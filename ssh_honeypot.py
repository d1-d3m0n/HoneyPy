    ...
    while True:
        ...
        if char == b'\r':
            if command.strip() == b'exit':
                ...
                ssh_logger.info(f'{client_ip} executed command: {command.strip().decode()}')
...
class Server(paramiko.ServerInterface):
    ...
    def check_auth_password(self,username,password):
        ssh_logger.info(f'{self.client_ip} attempted SSH login with username="{username}" password="{password}"')
...
def client_handle(client,addr,username,password):
    ...
    try:
        ...
        standard_banner = "Welcome to Ubuntu 22.04 LST(Jammy Jellyfish)!\r\n\r\n"
